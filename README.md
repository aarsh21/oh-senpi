...
For reproducible installs, append a release tag, for example `@v0.2.1`.
...
| **Subagents**                  | Pi, Claude Code, and Codex background subagents, `/subagents`, and `/btw` side questions | Pi backend works in-process. Claude backend needs Claude Code auth; Codex backend needs the `codex` CLI and auth.                      |
